...
public class ASMCodeGenerator implements CodeGenerator {
    ...
    private final List<OperatorToken> tokenList = new ArrayList<OperatorToken>();
    ...
    // 标记Token，用于中间结果表示
    private static final Token<?> MARK_TOKEN = new NumberToken(new Double(0), -1);


    private int getStoreIndex(TokenType tokenType) {
        int offset = 1;
        switch (tokenType) {
        case Number:
            offset = 2;
    ...
    private void doArthOperation(String method) {
        popOperand();
        popOperand();
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/googlecode/aviator/runtime/type/AviatorObject", method,
    ...
    private void doCompareAndJump(int ints) {
        ...
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/googlecode/aviator/runtime/type/AviatorObject", "compare",
            "(Lcom/googlecode/aviator/runtime/type/AviatorObject;)I");
        Label l0 = makeLabel();
        Label l1 = makeLabel();
    ...
    public void onNeg(Token<?> lookhead, int depth) {
        mv.visitTypeInsn(CHECKCAST, "com/googlecode/aviator/runtime/type/AviatorNumber");
        mv.visitMethodInsn(INVOKEVIRTUAL, "com/googlecode/aviator/runtime/type/AviatorNumber", "neg",
